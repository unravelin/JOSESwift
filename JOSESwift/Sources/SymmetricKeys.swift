...
struct SymmetricKey: JWK {
    ...
    init(key: Data, additionalParameters parameters: [String: String] = [:]) {
        self.keyType = .OCT
    ...
    /// - Throws: A `JOSESwiftError` indicating any errors.
    init(key: ExpressibleAsSymmetricKeyComponents, additionalParameters parameters: [String: String] = [:]) throws {
    ...
    /// - Throws: A `JOSESwiftError` indicating any errors.
    func converted<T>(to type: T.Type) throws -> T where T: ExpressibleAsSymmetricKeyComponents {
